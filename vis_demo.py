...
if True:
    ...
    Y = np.linspace(-5, 5, 100) # plot two lines meanwhile
    viz.line(
        Y=np.column_stack((Y * Y, np.sqrt(Y + 5))), # [100, 2]
        X=np.column_stack((Y, Y)), # [100, 2]
...
viz.line( # add a new trace, I dnt what's difference in append & insert
    X=np.arange(1, 10),
    Y=np.arange(11, 20),
    win=win,
    name='4',
...
viz.line(X=None, Y=None, win=win, name='delete this', update='remove') # remove a trace
