...
def main(args):
    ...
    mini = MiniImagenet('/home/i/tmp/MAML-Pytorch/miniimagenet/',
                        mode='train',
                        n_way=args.n_way,
                        k_shot=args.k_spt,
                        k_query=args.k_qry,
    ...
    # we mix the usage of epoch and batch here.
    for epoch in range(epoch_start, args.train_episode_num):

        try:
            x_spt, y_spt, x_qry, y_qry = iter(db).next()
        ...
        # [task_num, setsz, 3, 64, 64]
        # [task_num, qrysz, 3, 64, 64]
        x_spt, y_spt, x_qry, y_qry = x_spt.to(device), y_spt.to(device), x_qry.to(device), y_qry.to(device)


        if args.is_meta: # for meta
            loss_optim, losses_q, likelihoods_q, klds_q = net(x_spt, y_spt, x_qry, y_qry)
            ...
            loss_optim, _, likelihood, kld = net(x_spt, y_spt, x_qry, y_qry)
            optimizer.zero_grad()
            loss_optim.backward()
        ...
        if epoch % 50 == 0:
            ...
            x_hat = net.forward_ae(x_qry[0])
            vis.images(x_qry[0], nrow=args.k_qry, win='train_x', opts=dict(title='train_x'))
